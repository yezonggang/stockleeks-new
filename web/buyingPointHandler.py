...
# 获得页面数据。
class GetBuyingPointHtmlHandler(webBase.BaseHandler):
    @gen.coroutine
    def get(self):
        ...
        try:
            ...
            print("error :", e)
        logging.info("####################GetBuyingPointHtmlHandler")
        self.render("stock_buying_point.html", stockWeb=stock_web, date_now=date_now_str,
                    pythonStockVersion=common.__version__,
                    leftMenu=webBase.GetLeftMenu(self.request.uri))


# 获得股票数据内容。
class GetBuyingPointDataHandler(webBase.BaseHandler):
    def get(self):
        print("get start")
        ...
        search_sql = search_sql + " AND `code` IN (SELECT gs.`code` FROM guess_indicators_lite_sell_daily gs WHERE gs.date> DATE_SUB(CURDATE(),INTERVAL 1 YEAR))"
