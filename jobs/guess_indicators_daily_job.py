...
def stat_all_lite_buy(tmp_datetime):
    ...
    sql_1 = """
            SELECT `date`, `code`, `name`, `quote_change`, `latest_price`,`ups_downs`,`amplitude`, `open`, `high`, `low`, 
                        ...
                        and kdjk >= 78 and kdjd >= 68 and kdjj >= 90
    """  # and kdjj > 100 and rsi_6 > 80  and cci > 100 # 调整参数，提前获得股票增长。and rsi_6 >= 78  and cci >= 98
...
def apply_guess(tmp, stock_column):
    ...
    date_start = (date_end + datetime.timedelta(days=-250)).strftime("%Y-%m-%d")
